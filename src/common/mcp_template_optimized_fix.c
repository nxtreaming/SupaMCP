...
static int mcp_template_pattern_match(const char* value, const char* pattern) {
...
// Internal function to validate a parameter value against a validation spec
static int validate_param_value(const char* value, const mcp_template_param_validation_t* validation) {
    ...
    switch (validation->type) {
        ...
        case MCP_TEMPLATE_PARAM_TYPE_CUSTOM:
            ...
            // Simple pattern matching with * wildcard
            return mcp_template_pattern_match(value, validation->pattern);
 ...
 * It handles the case where the static part is empty (at the end of the URI).
 *
