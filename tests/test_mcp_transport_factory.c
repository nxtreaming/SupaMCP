...
#include "transport/internal/transport_interfaces.h"
...
static int validate_transport_vtable(mcp_transport_t* transport, mcp_transport_type_t expected_type) {
    ...
    // Check that function pointers are not NULL based on transport type
    if (transport->type == MCP_TRANSPORT_TYPE_SERVER) {
        if (!transport->server.start || !transport->server.stop || !transport->server.destroy) {
            return 0;
        }
    } else { // MCP_TRANSPORT_TYPE_CLIENT
        if (!transport->client.start || !transport->client.stop ||
            !transport->client.send || !transport->client.destroy) {
            return 0;
        }
    }
