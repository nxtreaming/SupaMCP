...
// Helper to obtain a free ephemeral port for testing
static uint16_t get_free_port(void) {
    ...
    if (bind(sock, (struct sockaddr*)&addr, sizeof(addr)) == MCP_SOCKET_ERROR) {
        mcp_socket_close(sock);
        return 0;
    }

    if (getsockname(sock, (struct sockaddr*)&addr, &addr_len) == MCP_SOCKET_ERROR) {
        mcp_socket_close(sock);
        return 0;
    }
...
void test_tcp_transport_lifecycle(void) {
    ...
    TEST_ASSERT_NOT_EQUAL_MESSAGE(0, port, "Failed to obtain free port");
