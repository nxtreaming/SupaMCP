    ...
    *error_message = NULL;
    ...
    if (strcmp(name, "echo") == 0) {
        ...
        if (g_transport) {
            ...
            mcp_http_transport_send_sse(g_transport, "echo", event_data);
        }
        ...
        if (result_data != NULL) {
            ...
            if (g_transport) {
                ...
                mcp_http_transport_send_sse(g_transport, "reverse", event_data);
            }
        }
    } else {
...
int main(int argc, char** argv) {
    ...
    if (f) {
        ...
        fprintf(f, "    <pre>curl -X POST http://%s:%d/call_tool -H \"Content-Type: application/json\" -d \"{\\\"name\\\":\\\"echo\\\",\\\"params\\\":{\\\"text\\\":\\\"Hello, MCP Server!\\\"}}\"</pre>\n", host, port);
        fprintf(f, "</body>\n");
        fprintf(f, "</html>\n");
        fclose(f);
